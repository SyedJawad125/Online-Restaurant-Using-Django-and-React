...
const Updaterestaurant = () => {
  ...
  const [opening_hours, setopening_hours] = useState(data.opening_hours)
  const [closing_hours, setclosing_hours] = useState(data.closing_hours)
  const [cuisine_type, setcuisine_type] = useState(data.cuisine_type)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {"id": data.id, "name":name , "address":address, "phone_number":phone_number , "email":email , 
        "website":website, "opening_hours":opening_hours, "closing_hours":closing_hours, "cuisine_type":cuisine_type}
      
      const response = await axiosInstance.patch('/restaurent/restaurant', payload , {
        headers: {
      ...
      if (response){
        ...
        navigate('/restaurant', {state: {message: 'Restaurant Update!'}})
      }
      // Handle success
    } catch (error) {
  ...
  return (
    <div class='container' style={{ marginLeft: '200px' }}>
        <h2 class='mt-4'>Update Restaurant Here :</h2>
